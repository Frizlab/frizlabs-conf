...
## Usage: defaults_set_str domain key value
...
function defaults_set_str() {
	...
	defaults write "$domain" "$key" -string "$value" >/dev/null 2>&1 || { log_task_failure "cannot set string value for defaults domain $domain key $key"; echo "failed"; return }
	echo "changed"
}
...
function defaults_set_int() {
	...
	defaults write "$domain" "$key" -int "$value" >/dev/null 2>&1 || { log_task_failure "cannot set integer value for defaults domain $domain key $key"; echo "failed"; return }
	echo "changed"
}
...
## Usage: defaults_set_bool domain key value
...
function defaults_set_bool() {
	...
	defaults write "$domain" "$key" -bool "$value_to_set" >/dev/null 2>&1 || { log_task_failure "cannot set bool value for defaults domain $domain key $key"; echo "failed"; return }
	echo "changed"
}
