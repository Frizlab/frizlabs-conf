...
## Set a bool key for the given key using macOS defaults CLT.
...
function defaults_set_bool() {
	...
	defaults write "$domain" "$key" -bool "$value_to_set" >/dev/null 2>&1 || { log_task_failure "cannot set value for defaults domain $domain key $key"; echo "failed"; return }
	echo "changed"
}
