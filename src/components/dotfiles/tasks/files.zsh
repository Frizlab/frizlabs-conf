...
for file in "${DOTFILES__FILES[@]}"; do
	dest_file="$HOME/$file"
	# Find the correct destination link.
	# If the file exists with an "@$COMPUTER_GROUP" suffix before the extension, we take this value.
	repo_file="$COMPONENT_ROOT_FOLDER/files/_${file%.*}@$COMPUTER_GROUP.${file##*.}"
	test -f "$repo_file" || repo_file="$COMPONENT_ROOT_FOLDER/files/_$file"
