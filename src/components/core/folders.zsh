...
{ test "$HOST_OS" = "Darwin" && res_check "$res" &&   res="$(acl    "$CLT_DIR" "$expected_acl")"   && res_list+=("$res") } || true
{ test "$HOST_OS" = "Darwin" && res_check "$res" &&   res="$(flags  "$CLT_DIR" "hidden")"          && res_list+=("$res") } || true
