...
if test "$HOST_OS:$HOST_ARCH" != "Darwin:arm64"; then
	...
	for formula_name path_to_check in ${(kv)X86_SYSTEM_HOMEBREW_FORMULAE}; do
		start_task "install formula $formula_name";   catchout RES  install_brew_package "--force-arch" "x86_64" "$HOMEBREW_X86_SYSTEM_DIR" "$formula_name" "$path_to_check" "--formula";   log_task_from_res "$RES"
	done
fi
